...
public class HomeController {
    ...
    @FXML
    private JFXCheckBox checkboxAll, checkboxOnlyMine, checkboxJustFriends;
    ...
    private boolean postOnMyBoard = true;

    public void initIDsYourPosts() {
        IDsYourPosts = Database.getNumberOfPosts(person.getID(), person.getFriends());
    }

    public void initPosts() {
        vboxForPosts.getChildren().clear();

        loadCreateNewPost(person);
        reportsPost = Database.getReportsPost();
        loadPosts();
    }

    private void loadCreateNewPost(Person person) {
        VBox vbox = new VBox();
        ...
        JFXCheckBox checkBox = new JFXCheckBox("Post on my board");
        HBox.setMargin(checkBox, new Insets(27, 0, 0, 5));
        ...
        button.addEventHandler(ActionEvent.ACTION, (ActionEvent e) -> {
            if (textArea.getText().length() == 0) {
                ...
                if(Database.addPost(person.getID(), textArea.getText(), postOnMyBoard)) {
                    NotificationBuilder.showNotificationSuccessOperation("Send post",
                            "Your post has been added!");
                } else {
                    NotificationBuilder.showNotificationSuccessOperation("Send post",
                            "Your post has been not added!");
                }
            }

            new LoadPage().loadHome(person);
        });
        HBox.setMargin(button, new Insets(10, 0, 0, 90));
        ...
        HBox.setMargin(btnUploadText, new Insets(35, 0, 0, 45));
    ...
    public void checkboxAll() {
        ...
        if (checkboxOnlyMine.isSelected()) {
            checkboxOnlyMine.setSelected(false);
            checkboxOnlyMine.setDisable(false);
        } else if (checkboxJustFriends.isSelected()) {
            checkboxJustFriends.setSelected(false);
            checkboxJustFriends.setDisable(false);
        }
    ...
    @FXML
    public void checkboxOnlyMine() {
        IDsYourPosts = Database.getNumberOfPostsForPerson(person.getID());

        checkboxOnlyMine.setDisable(true);
    ...
    @FXML
    public void checkboxJustFriends() {
        IDsYourPosts = Database.getNumberOfPostsMyFriends(person.getFriends());

        checkboxJustFriends.setDisable(true);

        if (checkboxAll.isSelected()) {
            checkboxAll.setSelected(false);
            checkboxAll.setDisable(false);
        } else if (checkboxOnlyMine.isSelected()) {
            checkboxOnlyMine.setSelected(false);
            checkboxOnlyMine.setDisable(false);
        }
